...
async def upload_project(
    ...
    """Upload a ZIP and persist ownership to either the user or guest session."""
    guest = None
    if current_user is None:
        if not guest_session_id:
            raise HTTPException(status_code=401, detail="Authentication or a guest session is required.")
        guest = db.query(Guest).filter(Guest.session_id == guest_session_id).first()
        if guest is None:
            raise HTTPException(status_code=401, detail="Guest session is invalid or expired.")
    ...
    try:
        ...
        workspace.cleanup()
        raise HTTPException(status_code=422, detail=f"ZIP validation failed: {exc}") from exc
    ...
    return ProjectUploadResponse(
        project_id=project.id, name=project_name, file_count=file_count,
        size_bytes=len(data), workspace_id=workspace.workspace_id,
